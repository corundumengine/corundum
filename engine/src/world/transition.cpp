...
namespace corundum::world {
  ...
  namespace {
    ...
    void SceneTransitioner::fail(std::string_view context, std::string_view err) noexcept {
      warn_log("[engine] {} failed: {}", context, err);
      engine_->request_quit();
    ...
    void SceneTransitioner::handle_map_transition() noexcept {
      try {
        if (!engine_->scene.pending_transition)
          return;
        const auto t = std::move(*engine_->scene.pending_transition);
        ...
        // Best-effort: a transition whose processing throws (e.g. allocation failure)
        // is dropped rather than terminating the noexcept main loop.
        return;
      }
    }
    ...
    std::expected<void, std::string> SceneTransitioner::apply_spawn(std::string_view mode, std::string_view id,
      ...
      const auto &new_tm = *engine_->active_tilemap();
