...
namespace corundum::dialogue {
  ...
  namespace {
    ...
    class Parser {
    public:
      Parser(std::string_view src, const corundum::world::FlagStore &vars, const corundum::quest::Registry *quests)
    ...
    private:
      ...
      int parse_primary() {
        switch (cur_.kind) {
          ...
          case TokKind::Ident: {
            ...
            return corundum::world::visit_count(vars_, name);
          }
          case TokKind::LParen: {
            advance();
      ...
      int parse_quest_helper(const std::string &name) {
        ...
        if (name == "quest_is_started") {
          expect_rparen();
          const auto key = corundum::quest::quest_flag_key(quest_id);
          return corundum::world::visit_count(vars_, key) > 0 ? 1 : 0;
        }

        if (name == "quest_is_resolved") {
          expect_rparen();
          const auto *q = quests_ ? quests_->find(quest_id) : nullptr;
          return (q && corundum::quest::is_complete(*q, vars_)) ? 1 : 0;
        }

        if (name == "quest_is_failed") {
          expect_rparen();
          const auto *q = quests_ ? quests_->find(quest_id) : nullptr;
          return (q && corundum::quest::is_failed(*q, vars_)) ? 1 : 0;
        }
        ...
        if (name == "quest_is_at") {
          ...
          if (!s)
            return 0;
          const auto key = corundum::quest::quest_flag_key(quest_id);
          return corundum::world::visit_count(vars_, key) == s->sequence ? 1 : 0;
        }

        if (name == "has_item") {
          expect_rparen();
          return corundum::world::visit_count(vars_, "item." + quest_id) > 0 ? 1 : 0;
        }

        if (name == "item_count") {
          expect_rparen();
          return corundum::world::visit_count(vars_, "item." + quest_id);
        }

        if (name == "rep") {
          expect_rparen();
          return corundum::world::visit_count(vars_, "rep." + quest_id);
        }
      ...
      const corundum::quest::Registry *quests_;
    };
