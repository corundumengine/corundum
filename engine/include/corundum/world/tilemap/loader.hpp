...
namespace corundum::world::tilemap {
  ...
  /// @pre @p path must refer to a readable, valid-JSON file. Tileset `source` paths are resolved
  ///      relative to the current working directory, so they must be reachable from there.
