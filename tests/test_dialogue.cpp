...
TEST_CASE("Graph::find returns correct node for known id") {
  const auto g = make_innkeeper_graph();
  const auto *n0 = g.find("n0");
  REQUIRE(n0 != nullptr);
  CHECK(n0->id == "n0");
...
TEST_CASE("Graph::find returns nullptr for unknown id") {
  const auto g = make_innkeeper_graph();
  CHECK(g.find("does_not_exist") == nullptr);
  CHECK(g.find("") == nullptr);
}
...
TEST_CASE("visible_choices: quest-gated choice shown when registry present and stage matches") {
  ...
  // reaches the compiled expression evaluator the same way the system side already does.
  using namespace corundum::dialogue;
...
TEST_CASE("divert: goto_graph records a visit for the entry node, not the graph's first node") {
  using namespace corundum::dialogue;

  // Spoke whose goto target ("b1") is the second node, so entry differs from nodes[0].
  Graph spoke;
  spoke.graph_id = "spoke";
  spoke.speaker = "Spoke";
  {
    Node n;
    n.id = "b0";
  ...
  corundum::dialogue::Registry graphs;
  graphs.add(make_hub_graph("spoke", "b1"));
  graphs.add(std::move(spoke));
  ...
  REQUIRE(conversation.graph_id() == "spoke");
  REQUIRE(conversation.current_node_id() == "b1");
  CHECK(corundum::world::visit_count(flags, visit_flag_key("spoke", "b1")) == 1);
  CHECK(corundum::world::visit_count(flags, visit_flag_key("spoke", "b0")) == 0);
}

TEST_CASE("divert: events emitted alongside a goto_graph reach the caller") {
  using namespace corundum::dialogue;
  ...
  hub.speaker = "Hub";
  {
    ...
    n.text = "Hello.";
    n.next_id = "a_divert";
    push_node(hub, std::move(n));
  }
  {
    Node n;
    n.id = "a_divert";
    n.type = NodeType::Event;
    n.actions = {"play_sound('ding')", "goto_graph('spoke', 'b0')"};
    n.next_id = "a1";
    push_node(hub, std::move(n));
  }
  {
  ...
  REQUIRE(events.size() == 1);
  CHECK(events[0].name == "play_sound");
  CHECK(events[0].args[0] == "ding");
  CHECK(conversation.graph_id() == "spoke");
  CHECK(conversation.current_node_id() == "b0");
}

TEST_CASE("divert: events emitted alongside a goto_graph on a choice edge reach the caller") {
  using namespace corundum::dialogue;
  ...
  hub.speaker = "Hub";
  {
    ...
    n.type = NodeType::Choice;
    n.choices = {
        {.label = "Go.", .target_id = "a1", .actions = {"play_sound('ding')", "goto_graph('spoke', 'b0')"}},
    };
    push_node(hub, std::move(n));
  }
  {
  ...
  CHECK(conversation.graph_id() == "spoke");
  CHECK(conversation.current_node_id() == "b0");
  CHECK(conversation.resume_node_id() == "a1");
}
...
TEST_CASE("Talk once: loads from JSON and round-trips through serialize") {
  ...
  REQUIRE(reloaded.has_value());
  CHECK(reloaded->find("n0")->once == true);
...
TEST_CASE("dialogue serialize round-trips through load_graph") {
  ...
  const auto *n0 = g2.find("n0");
  REQUIRE(n0 != nullptr);
  CHECK(n0->type == corundum::dialogue::NodeType::Talk);
  CHECK(n0->next_id == "n1");

  const auto *n1 = g2.find("n1");
  REQUIRE(n1 != nullptr);
  CHECK(n1->type == corundum::dialogue::NodeType::Choice);
  CHECK(n1->choices.size() == 3);
  CHECK(n1->choices[0].condition->source() == "gold >= 5 && !paid_innkeeper");
  ...
  const auto *n_pay = g2.find("n_pay");
  REQUIRE(n_pay != nullptr);
  CHECK(n_pay->type == corundum::dialogue::NodeType::Event);
  CHECK(n_pay->actions[0] == "play_sound('coin')");
  CHECK(n_pay->next_id == "n2");
