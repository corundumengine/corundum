...
TEST_CASE("pick_tile — flat map resolves the tile under the cursor") {
  ...
  const auto result = pick_tile(0.f, 16.f, camera, map, k_elev_step);
...
TEST_CASE("pick_tile — applies camera offset before inverting") {
  ...
  const auto result = pick_tile(0.f - 10.f, 16.f + 5.f, camera, map, k_elev_step);
  REQUIRE(result.has_value());
  CHECK(result->col == 0);
  CHECK(result->row == 0);
}
...
TEST_CASE("pick_tile — out of bounds returns nullopt") {
  ...
  const MapView map = make_view(tm);
  const Camera camera{0.f, 0.f};
  const auto result = pick_tile(10000.f, 10000.f, camera, map, k_elev_step);
